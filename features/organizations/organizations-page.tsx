...
const schema = z.object({ name: z.string().min(3), slug: z.string().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/, "Use letras minúsculas, números e hífens"), managerName: z.string().min(3), managerEmail: z.email(), managerTemporaryPassword: z.string().min(12), primaryColor: z.string().optional(), secondaryColor: z.string().optional() });
...
export function OrganizationsPage() {
  ...
  const create = useMutation({ mutationFn: organizationService.create, onSuccess: async () => { toast.success("Organização e manager inicial criados"); form.reset(); setShowForm(false); await refresh(); }, onError: (error) => toast.error(error.message) });
  ...
  return <><PageHeading eyebrow="Administração da plataforma" title="Organizações" description="Cadastre clientes e controle o acesso sem entrar nos dados internos de cada organização." action={<Button onClick={() => setShowForm((value) => !value)}><Plus size={17} /> Nova organização</Button>} />
    <Card className="admin-principle"><ShieldAlert /><div><strong>Separação preservada</strong><p>Esta área não cria links para campanhas, pessoas, ações ou métricas internas das organizações.</p></div></Card>
    {showForm ? <Card className="inline-form-card"><div className="card-heading"><div><p className="eyebrow">Novo cliente</p><h3>Organização e acesso inicial</h3></div></div><form className="form-grid" onSubmit={form.handleSubmit((values) => create.mutate(values))}><Field label="Nome"><Input {...form.register("name")} /></Field><Field label="Slug" error={form.formState.errors.slug?.message}><Input placeholder="minha-organizacao" {...form.register("slug")} /></Field><Field label="Nome do manager"><Input {...form.register("managerName")} /></Field><Field label="E-mail do manager"><Input type="email" {...form.register("managerEmail")} /></Field><Field label="Senha temporária" error={form.formState.errors.managerTemporaryPassword?.message}><Input type="password" {...form.register("managerTemporaryPassword")} /></Field><Field label="Cor principal"><Input {...form.register("primaryColor")} /></Field><Field label="Cor secundária"><Input {...form.register("secondaryColor")} /></Field><div className="form-actions"><Button type="button" variant="secondary" onClick={() => setShowForm(false)}>Cancelar</Button><Button loading={create.isPending}>Criar organização</Button></div></form></Card> : null}
