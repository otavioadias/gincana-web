...
const schema = z.object({
  ...
  role: z.enum(["MANAGER", "VALIDATOR", "MEMBER"]),
  temporaryPassword: z.string().min(12, "Use pelo menos 12 caracteres"),
});
type Values = z.infer<typeof schema>;
const roleLabels: Record<MembershipRole, string> = { MANAGER: "Gestão", VALIDATOR: "Validação", MEMBER: "Participante" };
...
export function MembersPage() {
  ...
  return (
    <>
      <PageHeading eyebrow="Pessoas que fazem acontecer" title="Equipe" description="Gerencie participantes, responsáveis pela validação e pessoas gestoras." action={<Button onClick={() => setShowForm((value) => !value)}><Plus size={17} /> Adicionar pessoa</Button>} />
      {showForm ? <Card className="inline-form-card"><div className="card-heading"><div><p className="eyebrow">Novo acesso</p><h3>Adicionar à equipe</h3></div></div><form className="form-grid" onSubmit={form.handleSubmit((values) => create.mutate(values))}>
        ...
        <Field label="Perfil"><select className="input" {...form.register("role")}><option value="MEMBER">Participante</option><option value="VALIDATOR">Validação</option><option value="MANAGER">Gestão</option></select></Field>
          ...
          return <div className="person-row" key={member.id}>
            ...
            <div className="person-actions">
              <Button variant="ghost" title="Redefinir acesso" onClick={() => {
                const temporaryPassword = window.prompt("Digite uma nova senha temporária (mínimo 12 caracteres):");
